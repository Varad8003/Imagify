    ...
    <div className=" fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center">
        ...
        >
           ...
           { state!='Login' &&  <div  className="border px-6 py-2 flex items-center gap-2 rounded-full mt-4">
                <img  className="outline-none text-sm" src={assets.user_icon} width={16} alt="" />
                <input onChange={e=>setName(e.target.value)} value={name} type="text" placeholder="Full Name" />
            </div>}
            <div  className="border px-6 py-2 flex items-center gap-2 rounded-full mt-4">
                <img  className="outline-none text-sm" src={assets.email_icon} alt="" />
                <input onChange={e=>setEmail(e.target.value)} value={email} type="email" placeholder="Email id" />
            </div>
            <div  className="border px-6 py-2 flex items-center gap-2 rounded-full mt-4">
                <img  className="outline-none text-sm" src={assets.lock_icon}   alt="" />
            ...
            :<p className="mt-5 text-center">Already have an accoutn ? <span className="text-blue-600 cursor-pointer" onClick={()=>setState('Login')}>Login</span> </p>}
